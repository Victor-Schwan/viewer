...
MyViewerProc::MyViewerProc() : marlin::Processor("MyViewerProc"), EventDisplayer(this) { _description = ""; }
...
void MyViewerProc::processEvent(EVENT::LCEvent* evt)
{
  ...
  printVisible("NUMBER OF CONFORMAL TRACKS: " + std::to_string(numConformalTracks) + "\n" + "NUMBER OF TPC TRACKS " +
               std::to_string(numClupatraTracks));
  std::vector<std::string> subDetNames;
  std::vector<std::string> ss = conformalTracks->getParameters().getStringVals("TrackSubdetectorNames", subDetNames);
  std::cout << std::to_string(subDetNames.size()) << '\n';
  std::cout << std::to_string(ss.size()) << '\n';
  for (const std::string& str : subDetNames)
  {
    std::cout << str << '\n';
  }
  printStars();
